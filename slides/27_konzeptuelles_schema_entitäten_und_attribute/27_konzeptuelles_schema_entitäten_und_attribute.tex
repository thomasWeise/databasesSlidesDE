...
%
\begin{itemize}%
\item Die erste wichtige Komponente, die wir modellieren müssen, sind die Datenstrukturen, die wir in der Datenbank speichern wollen.%
%
...
Sie kann von allen anderen Objekten unterschieden werden.}%
\end{definition*}%
%
\uncover<5->{%
\begin{itemize}%
...
\label{def:attribute}%
Ein \emph{Attribut}~\inEN{attribute}~$a$ ist charakterisiert mit einem Namen und einer Domäne~\attrDomainb{a} mit Werten die es annehmen kann.%
\end{definition*}%
%
\uncover<2->{%
\begin{itemize}%
\item Attribute are sind Features und Charakteristiken von Entitäten.%
%
...
\uncover<2->{%
\begin{itemize}%
\item Wir können die Konzepte \emph{Domäne} und \emph{Datentyp} im Kontext von konzeptuellen Modellen unterscheiden\cite{S2024D:LDMRMRA}.%
%
...
%
\uncover<5->{%
\bestPractice{entityTypeSingleObjekt}{%
Jede Entität (und jeder Entitätstyp) soll genau ein Objekt (einen Objekttyp) aus der Realität modellieren (und nicht mehr als einen)\cite{SS2005EIDDDFDB:NQORD}.%
}%
%
\uncover<6->{%
\begin{itemize}%
...
%
\item<7-> Wenn wir stattdessen den Namen des Fahrers und den Typ und die Eigenschaften der Autos in einen gemeinsamen Entitätstyp modellieren, dann werden wir wahrscheinlich Redundanz erzeugen.%
%
...
%
\item<4-> Die Module \citetitle{programmingWithPython}\cite{programmingWithPython} und \citetitle{databases}\cite{databases} zusammen bilden eine Entitätenmenge, welche wiederum eine Untermenge des Entitätstyps für Module ist.%
%
...
%
\item<2-> Wir könnten uns Entitätstypen wie Klassen vorstellen und Entitäten wir deren Instanzen.%
%
...
\begin{itemize}%
%
\item die ursprüngliche Notationen von \citeauthor{B1969DSD}\cite{B1969DSD} und \citeauthor{C1975TRMTAUVOD}\cite{C1975TRMTAUVOD,C1976TERMTAUVOD} werden noch immer verwendet\only<-4>{.}\uncover<5->{,}%
%
...
%
\item<3-> Wir wollen nun den Entitätstyp \emph{Student} modellieren.%
%
...
%
\item<14-> Jetzt klicken wir auf das Verbindungs-Symbol in der Palette und ziehen es auf das Attribut.%
}%
%
\only<-15>{%
...
\item<4-> Um dem Rechnung zu tragen, sollte das \emph{Name}-Attribut kein einzelnes Attribut sein.%
%
\item<5-> Wir könnten es in ein \emph{zusammengesetztes Attribut} umbauen, dass aus einem Vorname~\inEN{given name}, Familienname~\inEN{family name} und einer Anrede~\inEN{salutation} besteht.%
%
...
\item<9-> Ein mehrwertiges Attribut wird in \glsShort{ERD}s durch zwei ineinanderliegende Ellipsen dargstellt.%
...
%
\item<2-> Sie haben wahrscheinlich keine chinesische Ausweisnummer~(中国公民身份号码\cite{GB116431999CIN}).%
%
...
\begin{itemize}%
\only<-7>{%
\item Wir bemerken auch, dass \emph{Address} wahrscheinlich auch ein zusammengesetztes Attribut seien sollte.%
...
\item<6-> Ein automatisches System kann Informationen die feiner auflöst als eine Postleitzahl wahrscheinlich sowieso nicht sinnvoll verarbeiten.%
%
...
%
\item<4-> Sie ist ein Tupel aus ihren Attributwerten.%
%
...
%
\only<-15>{%
\item<6-> Es ist nicht immer klar, wie ein Attribut modelliert werden soll.%
}%
%
\item<7-> Zum Beispiel haben unsere Studenten ein Attribut \glsFull{dateOfBirth}.%
%%
\item<8-> Technisch haben wir bereits gelernt, dass ein Datum ein atomarer Datentyp in \glsShort{SQL} ist.%
%
\item<9-> Wir können \glsShort{dateOfBirth} also durchaus als einfaches Attribut modellieren.%
%
\item<10-> Wir könnten es aber auch als zusammengesetztes Attribut aus Jahr, Monat, und Tag modellieren.%
%
\item<11-> Das würde allerdings kein vernünftig denkender Mensch machen, denn dann muss man ja auch Dinge wie Zeitzonen und verschiedene Kalender beachten\dots%
%
\item<12-> Eine Adresse kann ein einfach Textstring oder ein zusammengesetztes Attribut sein.%
%
\item<13-> Die Entscheidung, wie wir Attribute modellieren, hängt oft von den Daten ab, die wir brauchen.%
%
\item<14-> Wenn wir das Geburtsdatum als einfaches Attribut vom Datentyp \inQuotes{Datum} speichern, dann kann wir sehr einfach das Geburtsjahr oder das Alter einer Person berechnen.%
%
\item<15-> Speichern wir die Adresse jedoch als einfaches Attribut, dann wird es eben sehr schwer, das Land, die Provinz, oder die Stadt automatisch zu extrahieren.%
%
\item<16-> Attribute können zusätzlich noch einwertig oder mehrwertig sein, optional oder nicht-optional.%
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
%
\item<3-> Es geht auch um das Einschränken der Daten und die Beziehungen zwischen den Daten.%
%
