...
\toggleToGerman%
%
\subtitle{24.~Datenbanken Entwicklung}%
%
\begin{document}%
%
\startPresentation%
...
\begin{itemize}%
\item In unserem einführenden Fabrik-Beispiel hatten wir eine einfache Idee, wie unsere \glslink{db}{Datenbank} aussehen sollten.%
%
...
%
\item<7-> Wir haben eine grobe Idee, welche technologische Werkzeuge wir zur Verfügung haben, wenn wir mit \pglspl{db} haben.%
%
...
%
\item<3-> Wir wissen, dass die Batterie macht und wie wir mit ihr den Motor zum Laufen bringen.%
%
...
Es representiert die Entitäten, ihre Attribute und Beziehungen sowie Einscharänkungen mit konkreten Datentypen in einem strukturierten formalen Format.}%
\end{definition*}%
%
\uncover<3->{%
\begin{itemize}%
...
%
\item<2-> Das konzeptuelle Model wird in eine technschie Spezifikation übersetzt.%
%
...
%
\item<4-> Dieses konzeptionelle Modell muss letztendlich auf ein physisches Modell abgebildet werden.%
%
...
%
\only<-11>{%
\item<2-> Anders als normale Softwareprojekte scheinen Datenbankapplikationen überide Jahre \emph{nicht} komplexer zu werden, sie entwickeln sich aber trotzdem immer weiter\cite{SVZ2014OSD}.%
}%
%
\only<-12>{%
...
%
\item<12-> Gleichzeitig sollte verhindern, dass iterative Veränderungen die Projektstruktur immer weiter verändern und das Projekt durch immer neue Features immer weiter vom Originalziel wegdriftet\cite{GMTM2011DDLC,WK1989DKBSRTSDLC}.%
\end{itemize}%
\end{frame}%
%
...
%
\item<7-> In manchen Varianten des Ansatzes werden die Projektanforderungen für festgelegt\cite{PA2009SETAP,GMTM2011DDLC}, in anderen können sie später bearbeitet werden\cite{L1989SETRP}.%
%
...
%
\item<13-> Drittens müssen wir immer beachten, dass Datenbanken sehr langlebige Artefakte sind, die über viele Jahre gewartet und verbessert werden müssen.%
%
...
%
\only<-12>{%
\item<9-> Die Entwickler und Designer arbeiten mit allen Stakeholders des Projekts auf allen ebenen zusammen, von den späteren Benutzern der Datenbank bis hin zum Management der Organisation.%
}%
%
\only<-13>{%
...
%
\only<-20>{%
\item<16-> In der Anforderungsanalyse untersichen wir die gsammelten Daten um zu entscheiden, ob das Projekt durchführbar ist und um seine Kosten abzuschätzen.%
}%
%
\only<-21>{%
...
%
\only<-41>{%
\item<37-> Lassen Sie uns trotzdem annehmen, dass wir uns für eine \glslink{rdb}{relationales} Datenbank entschieden haben.%
}%
%
\only<-42>{%
...
%
\only<-43>{%
\item<39-> Die Entscheidung wird getroffen basierend auf Kosten, Wrtung, Lizensen, und Trainingsmöglichkeiten.
}%
%
\only<-43>{%
...
%
\only<-44>{%
\item<41-> Dann wird das konzeptuelle Schmea in ein logisches Schema transformiert.%
}%
%
\only<-45>{%
...
%
\item<4-> Über hr Design und ihre Performanz haben sie Einfluss auf viele der wichtigstigen Prozesse einer Organisation.%
%
...
\begin{itemize}%
%
\item Die meisten Datenbank-Entwurfsprozesse arbeiten sich im Kern iterativ von einem konzeptuellen über ein logsches hin zu einem physischen Schema durch.%
%
...
%
\item<4-> Dabei können uns Prototypen helfen, Feedback zu sammeln und Missverständnisse zu entdeken.%
%
