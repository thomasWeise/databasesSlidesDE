...
\uncover<5->{%
\begin{itemize}%
\item Die Definition eines Relationsschemas im relationalen Modell ist daher in etwa Äquivalent zur Definition eines Entitätstypes im Entitätsmodell.%
%
...
\item jeder Wert~$F$ in einem Tupel~$r_1\in R_1$ entweder als Wert von $P$ für ein Tupel~$r_2\in R_2$ auftaucht oder \sqlilIdx{NULL}~ist.%
\end{enumerate}%
\end{definition*}%
%
\uncover<4->{%
...
\begin{itemize}%
\item In den 1980ern gab es viele Anbieter von \glsShort{dbms}en, die die Definition des relationalen Datenmodelss von \citeauthor{C1985IYDRR} nicht richtig implementiert hatten.%
%
...
Die Art wie ein \glsShort{dbms} Daten speichert hat keinen Anfluss darauf, wie Anwendungen auf die Daten mit Hilfe der text-basierten Sprache zugreifen.%
}}%
%
\setcounter{enumi}{8}%
