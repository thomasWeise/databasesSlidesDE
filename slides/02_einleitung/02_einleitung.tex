...
\item<2-> Name, Adressen, Bankkonten und -transaktionen,  Onlineeinkäufe, Zugtickets, Mobiltelefonnummern, \pgls{wechat}-Chats, Webseiten, Bücher, Gebrauchsanleitungen, Programmquelltexte, Karten, Schulnoten, Untersuchungsergebnisse und medizinische Historien, Steuerdaten, berufliche Werdegänge, Spielergebnisse{\dots}\uncover<3->{ Alles sind Daten.}%
\item<4-> Daten sind vielleicht die wichtigste Ressource unseres digitalen Zeitalters.%
...
\begin{itemize}%
\item Unstrukturierte Daten wie \DEzB\ Bücher, Bachelor- und Masterarbeiten, Berichte, Bugetpläne, und Forschungsanträge werden oft als einzelne Dokumente gespeichert.%
...
\item<2-> Dafür gibt es Formate wie \glsFull{XML}\cite{BPSMM2008EMLX1FE,K2019ITXJY,CH2013XFCAMLTMC}\uncover<4->{, \glsFull{JSON}\cite{E2017SE4TJDIS,RFC8259}\uncover<5->{, und \glsFull{YAML}\cite{DNMAASBE2021YAMLYV1,K2019ITXJY,CGTYB2022YFFDCAIE}.}}%
\end{itemize}%
%
...
Eine \emph{Datenbank}~(\glsShort{db}) ist eine Kollektion von in gegenseiter Beziehung stehenden Daten, die in einem Computer gespeichert sind, potentiell viele verschiedene Typen haben können, und auf die viele Nutzern und Applikationen gleichzeitig zugreifen können.%
\end{definition*}}%
\end{frame}%
%
\begin{frame}%
\frametitle{Datenbanken}
\begin{itemize}%
\item Es gibt viele verschiedene \glslink{db}{Datenbanken}.%
\item<2-> Datenbanken für Dokumente\uncover<3->{, für geographische Daten\uncover<4->{, für komplexe Objekte, usw.}}%
...
Eine relationale Datenbank~(\glsShort{rdb}) ist eine Datenbank, die Daten in Zeilen~(Rows, Tupel, Datensätze, Records) und Spalten~(Columns, Attribute) organisiert, die zusammen Tabellen~(Relationen) bilden wobei die Datenpunkte zu einander in Beziehungen stehen\cite{I2021WIARDB,C1970ARMODFLSDB,SC1975OTPOARADI,T2018ISARD}.%
\end{definition*}%
%
...
%
\begin{frame}%
\frametitle{Relational Daten}%
\begin{itemize}%
\item Ein Studenten-Datensatz könnte \DEzB\ den Namen des Studenten/in speichern, seinen/ihren Geburtstag, die Ausweisnummer, und die Mobiltelefonnummer.%
...
\item<5-> Ein Studenten-Datensatz könnte \DEzB\ mit einem Lehrer-Datensatz verbinden sein.%
...
Ein Datenbankmanagementsystem~(\glsShort{dbms}) ist ein Softwaresystem um mit Daten arbeiten zu können.
...
\begin{itemize}%
\item Ein \glsShort{dbms} muss es uns erlauben, Einschränkungen~\inEN{constraints} und Beziehungen für Daten zu spezifizieren.%
...
\item<4-> Das \glsShort{dbms} muss es uns ermöglichen, solche Einschränkungen und Beziehungen zu definieren und muss dann dafür sorgen, dass diese immer eingehalten werden.
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
\begin{itemize}%
\item Ein \glsShort{dbms} muss es uns auch erlauben, Regeln für den Datenzugriff zu formulieren.%
...
\begin{itemize}%
\item Ein \glsShort{dbms} muss es mehreren Personen und Applikationen ermöglichen, gleichzeitig auf die Daten zuzugreifen.%
\item<2-> Es muss verhindern, dass die Datenintegrität verletzt wird, wenn gleichzeitig mehrere Personen die Daten verändern.%
\item<3-> Manche Datenbanken sind so groß, dass sie über mehrere Computer in einem Cluster verteilt werden müssen.%
\item<4-> Das \glsShort{dbms} muss dann die Datenintegrität über alle Datenbankteile hinweg sicherstellen.%
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
Datenbankadministratoren~(\glsShort{dba}s) sind die Person oder Gruppe von Personen, die für die effektive Nutzung von Datenbanktechnologien in einer Organisation oder einem Unternehmen zuständig ist.%
\end{definition*}%
\uncover<3->{%
\begin{itemize}%
...
\item<4-> Wir schauen uns an, welche \glsShort{dbms}e es so gibt.%
