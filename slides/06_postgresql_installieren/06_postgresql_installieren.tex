...
\toggleToGerman%
%
\subtitle{6.~PostgreSQL Installieren}%
%
\begin{document}%
%
\startPresentation%
...
\end{itemize}%
\locateGraphic[The \href{https://www.postgresql.org}{\postgresql}~logo is under the copyright of its owners.]{}{width=0.225\paperwidth}{../05_software_und_literatur/graphics/postgresqlLogo}{0.7}{0.54}%
\end{frame}%
%
\begin{frame}
...
Das können wir durch das Kommando \bashil{systemctl status postgresql} tun, welches wir mit~\keys{\return} ausführen.}%
%
...
%
\only<8>{\item Wir drücken~\keys{q+\return} um das Kommande zu beenden.}%
%
...
Wir können seine Version mit dem Kommando \bashil{psql --version} herausfinden, welches wir mit~\keys{\return} ausführen.}%
%
...
Wir tuen dies durch \bashil{sudo -u postgresql psql} und drücken~\keys{\return}.}%
%
...
%
\only<15>{\item Wir drücken~\keys{\return} und das System bestätigt die Änderung, in dem es das Kommando \sqlil{ALTER ROLE} nochmal ausgibt. %
Von nun an hat das Hauptbenutzerkonto des Servers ein sicheres Passwort.}%
%
\only<16>{\item Wir verlassen \psql, in dem wir \keys{\textbackslash+q+\return} schreiben.}%
%
...
%
\only<15>{\item Mit der \pgls{locale} können wir länder- und kulturspezifische Einstellungen laden, \DEzB\ Nummern- und Währungsformate. %
Wir lassen die Standardeinstellungen unverändert und klicken auf~\menu{Next}.}%
%
...
%
\only<22>{\item Wir schreiben \textil{cmd} in die Eingabebox und drücken~\keys{\return}.}%
%
...
Wir geben es ein und drücken~\keys{\return}. %
Wir sind nun in der \psql-Konsole und sehen den \textil{postgres=\#}-Prompt.}%
%
\only<30>{\item Wir geben das SQL-Kommando \sqlil{SELECT * FROM VERSION();} ein und drücken~\keys{\return}.}%
%
\only<31>{\item Das Ergebnis zeigt in meinem Fall, das der \postgresql-Server auch Version~17.2 hat.%
Nun schreiben Sie \keys{\textbackslash+q+\return}, wodurch \psql\ verlassen wird.}%
%
