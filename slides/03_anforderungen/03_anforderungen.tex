...
\begin{itemize}%
%
\item Wir haben nun eine ungefähre Idee, was eine Datenbank~(\glsShort{db}) und was ein Datenbankmanagementsystem~(\glsShort{dbms}) sind.%
...
\item<4-> Stellen wir uns dafür vor, dass wir eine Datenbank für eine Bank entwickeln wollen.%
\item<5-> Datenbank soll die Informationen über die Kunden, ihre Konten, und Transaktionen speichern, sowie die Daten über die Angestellten der Bank.%
\item<6-> Davon ausgehend bauen wir uns eine Wunschliste von Features und Anforderungen zusammen.%
%
\end{itemize}%
\end{frame}%
%
...
\item<2-> Wenn wir mit einer Programmiersprache wie \python\ programmieren, dann können wir Datenstrukturen erstellen.
...
\item<5-> Also wir brauchen sowas auch für Datenbanken.%
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
\item<3-> Wir wollen definieren, welche Informationen wir über Kunden speichern, \DEzB\ ihre Namen, Ausweisnummern, Mobiltelefonnummern, usw.%
\item<4-> Wir müssen auch Einschränkungen~\inEN{constraints} für die Integrität der Daten defininieren, \DEzB\ daß Ausweisnummern dem Standard für Chinesische Identifikationsnummern~(中国公民身份号码)\cite{GB116431999CIN} entsprechen müssen, usw.%
\item<5-> Wir wollen auch Beziehungen zwischen den Tabellen spezifizieren, \DEzB\ daß jeder Eintrag in der Tabelle für Bankkonten mit genau einem Eintrag in der Tabelle für Kunden verbunden ist, aber das jeder Kunde mehrere Bankkonten haben darf.
...
\item<4-> Dafür gibt es darunterliegende \emph{physische Schema}.%
...
\begin{itemize}%
\item \alert{Konsistenz}:~Das DBMS stellt sicher, dass die modellierten Beziehungen und Einschränkungen für die Daten immer eingehalten werden.\uncover<3->{ Wenn wir festgelegt haben, dass ein Bankkonto immer einem Kunden gehört, dann darf das DBMS nie zulassen, das irgendwie ein Bankkonto entsteht, das nicht einem Kunden gehört.}%
...
\begin{enumerate}%
\item \alert{Atomizität}~\inEN{atomicity}%
\item \alert{Konsistenz}~\inEN{consistency}%
\item \alert{Isolation}~\inEN{isolation}%
\item \alert{Dauerhaftigkeit}~\inEN{durability}%
\end{enumerate}}%
\end{itemize}%
\end{frame}%
%
