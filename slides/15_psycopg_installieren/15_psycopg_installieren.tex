...
\end{frame}%
%
\section{Virtuelle Environments}%
%
\begin{frame}%
...
%
\only<-8>{%
\item<8-> Wir initialisieren ein leeres \glslink{virtualEnvironment}{virtuelles Environment} in diesem Ordnung mit dem Kommando \bashil{python3 -m venv --upgrade-deps .venv} und~\keys{\return}.%
}%
%
\only<-10>{%
\item<9-> Das \glslink{virtualEnvironment}{Virtuelle Environment} wurde erstellt.%
\item<10-> Wir aktivieren das Environment mit dem Kommando \bashil{source .venv/bin/activate} und~\keys{\return}.%
}%
%
\only<-13>{%
...
\item<16-> Wir installieren \psycopg\ in unser neues Environment in dem wir \bashil{pip install psycopg} schreiben und \keys{\return}~drücken.%
