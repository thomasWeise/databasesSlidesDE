...
%
\item<4-> Wir können Kontrollflussstatements wie~\pythonil{if...then...else}, \pythonilIdx{for}- und \pythonilIdx{while}~Schleifen verwenden.%
%
...
%
\only<-3>{%
\item<2-> Unser Programm muß dazu zwei Funktionen vom Paket \psycopg\ importieren, nämlich \pythonil{connect} und \pythonil{dict_row}.
\item<3-> Wir brauchen diese, um uns mit dem \postgresql\ \dbms\ zu verbinden und um zu definieren, wie wir Anfrageergebnisse empfangen wollen.%
}%
%
\only<-5>{%
...
%
\only<-19>{%
\item<18-> Dafür geben wir den Name der Tabelle (\sqlil{demand}) sowie die Namen der Spalten an, deren Werte wir schreiben wollen, nämlich \sqlil{customer}, \sqlil{product}, \sqlil{demand}, and \sqlil{ordered}.%
...
\listingOutput{25-}{factory:connect_and_insert_1}{style=text_style}{0.12}{0.42}{0.78}{0.7}%
%
\end{frame}%
%
...
\item<5-> Zweitens bekommt diese Methode eine \alert{Sequenz von} Sequenzen von Parameterwerten, wobei das \sql-Kommando dann für jede einzelne Sequenz aufgerufen wird.%
}%
%
\only<-7>{%
