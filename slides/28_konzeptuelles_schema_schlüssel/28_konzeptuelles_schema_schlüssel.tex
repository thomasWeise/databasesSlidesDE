...
%
\item<3-> Entitäten werden vollständig und ausschließlich von ihren Attributen charakterisiert.%
%
...
\item<5-> Oder nur durch die Mobiltelefonnummer(n) {\dots} also wenn diese nicht optional wären.%
...
%
\uncover<2->{%
\begin{definition*}[Primär-Attribut]%
Ein Attribut wird als primär~\inEN{prime} bezeichnet, wenn es Teil des Primärschlüssels ist.%
\end{definition*}%
%
\uncover<3->{%
%
...
\only<17-26>{%
\begin{itemize}%
\item Ein weiteres Kriterium für Primärschlüssel ist, dass sie \emph{klein} seien sollten.%
...
\end{itemize}}%
%
\item<27-> Bleibt also noch die von unserer Uni zugewiesene Studenten-ID.%
\only<28-34>{%
\begin{itemize}%
\item Sagen wir, dass eine Studentin einen Studiengang beginnt, vielleicht das Bachelor-Program \inQuotes{Informatik}.%
\item<29-> Dann bekommt sie eine Studenten-ID zugwiesen.%
\item<30-> Diese repräsentiert sie nicht als Person, sondern in der Funktion~\inQuotes{BSc-Student der Informatik}.%
\item<31-> Wenn Sie nach dem Abschluss einen Master ranhängen will, dann bekommt sie eine andere Studenten-ID.%
\item<32-> Das fühlt sich ein wenig komisch an.%
\item<33-> Vielleicht sollten wir Studenten irgendwie anders modellieren(?)%
\item<34-> Aber von den zur Verfügung stehenden Schlüsselkandidaten ist die Studenten-ID die beste Wahl.%
\end{itemize}%
}%
%
\item<35-> Wir nehmen die Studenten-ID als Primärschlüssel.%
}%
\item<36-> Wir können unser \glsShort{ERD} mit der Student-ID als Primärschlüssel updaten, in dem wir den Name des Attributs unterstrichen\cite{G2011EW2ITDS:CMUTERM}.%
%
\end{itemize}%
...
\item<3-> sich nicht ändern, so lange die Entität existiert\only<-3>{.}\uncover<4->{,}%
