...
%
\item<19-> Jede Nachricht kann die Antwort~\inEN{answer} auf entweder keine oder genau eine vorherige Nachricht sein.%
%
...
\item<10-> Aber auch schwachte Entitäten können mit schwachen Beziehungen verbunden sein~(so lange diese Beziehungen nicht benötigt werden, um die Entitäten zu identifizieren.)%
\end{itemize}%
%
}}}}%
\end{frame}%
...
%
\only<-29>{%
\item<21-> Die meisten \glsShort{dbms}e unterstützen nämlich \emph{Transactionen}, also Gruppen von Befehlen die als eine unteilbare~\inEN{atomic} Einheit ausgeführt werden und entweder zusammen erfolgreich sind oder zusammen fehlschlagen.%
}%
%
\only<-30>{%
\item<22-> Es gibt keinen Zwischenzustand, weil Transactionen unteilbar sind.%
}%
%
\only<-30>{%
...
%
\only<-20>{%
\item<15-> Es kann aber auch sein, dass manche Module bestimmte Zertifikate voraussetzen, vielleicht Chemnie-Sicherheits-Zertifikate oder so etwas.%
}%
%
\only<-21>{%
...
%
\only<-42>{%
\item<37-> Jede Vertifungsrichtung kann (rekursiv) eigene andere Pflicht- und Wahlfächer anbieten.%
}%
%
\only<-43>{%
...
%
\only<-69>{%
\item<64-> Diese Instanz ist immer mit einem Startsemester verbunden, \DEzB\ dem  Winter Semester 2025 oder vielleicht dem maybe dem Summer Semester 2026.%
}%
%
\only<-70>{%
...
%
\only<-80>{%
\item<75-> Kurse und Einschreibungen~\inEN{enrollments} sind schwachte Entitäten.%
}%
%
\only<-80>{%
...
%
\item<83-> Es kann ja den Studiengang sehen, zu dme die Studenten gehören, weiß welche Module Pflicht sind, und kann die Studenten dann automatisch eintragen, wenn es keine Auswahlmöglichkeit gibt.%
%
