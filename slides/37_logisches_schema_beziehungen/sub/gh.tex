...
%
\item<3-> Wir können auch keine Zeile einfügen, die keine existierende Zeile in Tabelle~\sqlil{h} referenziert, die noch mit keiner Zeile in Tabelle~\sqlil{g} verbunden ist.%
%
...
%
\only<-30>{%
\item<26-> Eine Idee wäre es, zu versuchen eine Anfrage wie folgt zu bauen: \sqlil{UPDATE h} \sqlil{SET fkgid =} \sqlil{(INSERT INTO g} \sqlil{(fkhid, x)} \sqlil{VALUES (1, '123')} \sqlil{RETURNING gid, fkhid)} \sqlil{WHERE h.hid = fkhid;}.%
}%
%
\only<-31>{%
