...
\uncover<2->{%
\begin{itemize}%
\item Es kann binäre Beziehungen gegebn, also solche, wo zwei Entitäten teilnehmen.\uncover<3->{ %
...
\item<5-> In dem \glsShort{ERD} hier sehen, dass der Entitätstyp \emph{Student} mit dem Entitätstyp \emph{Modul} über die Beziehung \emph{enrolls into}~(also \emph{schreibt sich ein}) verbunden ist.%
...
\only<-9>{%
\item<3-> Das taucht hier nicht auf\only<-3>{.}\uncover<4->{ Das Diagramm sagt gar nichts über die Beziehung von Student, Professor, und Modul aus.}%
}%
\only<-10>{%
...
\item<5-> Leute können nähmlich mehrere Namane haben.%
}\only<-17>{%
...
%
\only<-20>{%
\item<16-> Wir führen den neuen Entitätstype \emph{Person} ein.%
}\only<-21>{%
...
%
\item<10-> Wenn ein Ausländer China betritt, dann braucht sie Visum, welches wiederum eine einmalige Nummer hat und eine Gültigkeitsdauer\cite{CMOFA2019ITCV}.%
%
...
%
\item<2-> Eine Entät dieses Typs speichert den Name des ID-Typs, welcher der Primärschlüssel ist.%
%
