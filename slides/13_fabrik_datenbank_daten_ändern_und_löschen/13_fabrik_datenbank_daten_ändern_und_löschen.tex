...
\toggleToGerman%
%
\subtitle{13.~Fabrik-Datenbank:~Daten ändern und löschen}%
%
\begin{document}%
%
\startPresentation%
...
\listingSQLandOutput{1}{factory:update_table_product_1}{}{0.13}{0.17}{0.9}{0.81}
\gitLoadAndExecSQL{factory:update_table_product_2}{}{factory}{update_table_product_2.sql}{factory}{boss}{superboss123}%
\listingSQLandOutput{2}{factory:update_table_product_2}{}{0.13}{0.17}{0.9}{0.81}
...
\listingSQLandOutput{3}{factory:update_table_product_4}{}{0.13}{0.17}{0.9}{0.81}
\end{frame}
%
\begin{frame}[t]
...
\item<5-> Was passiert, wenn wir die \sqlil{product}~Tabelle so ändern, dass \inQuotes{Shoe, Size 42} dann \sqlil{id = 20} hat?\uncover<6->{ Die Zeile mit der Bestellung in Tabelle~\sqlil{demand} würde dann ja nicht mehr auf ein existierendes Produkt verweisen\dots}}%
\item<7-> Wir fragen also: Können wir mit \sqlil{UPDATE} die referentielle Integrität unserer Daten verletzen?%
...
\listingSQL{8}{factory:update_table_product_error}{0.05}{0.3}{0.9}{0.79}
\listingOutput{9}{factory:update_table_product_error}{style=tool_style_sql}{0.05}{0.3}{0.9}{0.79}
\end{frame}
%
...
\item<5-> Was passiert, wenn wir diesen Eintrag aus der Tabelle~\sqlil{customer} löschen?\uncover<6->{ Die Zeilen mit seinen Bestellungen in Tabelle~\sqlil{demand} würde dann ja nicht mehr auf einen existierenden Kunden verweisen\dots}}%
...
\item<7-> \postgresql\ bietet \DEzB\ zusätzlich das Statement~\sqlil{RETURNING} für die \sqlil{INSERT}, \sqlil{UPDATE}, und \sqlil{DELETE}-Befehle an, welches sehr nützlich ist und uns jeweils die betroffenen Zeilen der Tabelle zurückgibt.%
