...
\item<2-> In der ähnlichen Modellierungssprache \pgls{UML} bezeichnet man die Kardinalität als Multiplizität~\inEN{multiplicity}.%
\end{itemize}%
}%
\end{frame}%
%
...
%
\item<5-> \microsoftAccess\ offers bietet eine ähnliche Funktionalität, abre die Beziehungsenden sind mit~1 oderr~$\infty$ annotiert.%
%
...
\item<19-> \emph{Beispiel:} Ein Trainer einer Fußballmannschaft kann mehrere Klubmitglieder trainieren, aber immer mindestens eins, sonst ist er kein Trainer. %
...
\only<-24>{%
\item<23-> \emph{Beispiel:} Polizeipatrouillen werden von Teams zweier Polizeibeamte durchgeführt~(zumindest in Filem\dots).%
}%
...
%
\only<-13>{%
\item<11-> Weil Studiengangsinstanzen keine anderen identifizierenden Characteristika haben und nicht \inQuotes{losgelöst} existieren können, sind sie schwache Entitäten.%
}%
%
\only<-15>{%
