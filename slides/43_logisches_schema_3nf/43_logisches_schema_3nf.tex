...
\uncover<4->{%
\only<-7>{%
\begin{definition*}[Transitive Functionale Abhängigkeit]%
...
\uncover<7->{%
\begin{itemize}%
\item Eine Tabelle ist in der \glsShort{3NF}, wenn sie in der \glsShort{2NF} ist und alle Attribute, die nicht Teil eines Schlüsselkandidats sind, direkt vom Primärschlüssel abhängen.%
%
...
%
\only<-13>{%
\item<4-> Ein Kollege schlägt vor, dass es eine gute Idee wäre, auch die Kontaktinformation der Elten der Studenten zu speichern.%
}%
%
\only<-14>{%
...
%
\only<-14>{%
\item<7-> Dann speichern wir noch den name der Eltern und deren Mobiltelefonnummer.%
}%
%
\only<-15>{%
...
%
\item<8-> Deshalb hat er sich entschlossen, den Vater der drei Studenten kruzerhand \inQuotes{Herr Boddo} zu nennen.%
%%
\end{itemize}%
}%
%
...
%
\only<-18>{%
\item<13-> Die zweite Spalte dieser Tabelle ist \sqlil{name} -- und die hängt natürlich vom Primärschlüssel ab.%
}%
%
\only<-18>{%
...
%
\only<-13>{%
\item<4-> Sagen wir, dass es eine funktionale Abhängigkeit der Spalten~$A$ auf die Spalten~$B$ gibt und das diese transitiv auf den Schlüssel ist, also das wir \funcDepb{$K$}{\funcDepb{$B$}{$A$}} habe.%
}%
%
\only<-13>{%
