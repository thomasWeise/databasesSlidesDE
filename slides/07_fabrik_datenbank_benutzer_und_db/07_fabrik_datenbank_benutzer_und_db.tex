...
\toggleToGerman%
%
\subtitle{7.~Fabrik-Datenbank:~Benutzer und Datenbank}%
%
\begin{document}%
%
\startPresentation%
...
\only<6>{\item<6-> Wir wollen uns die Liste aller Benutzerkonten ausgeben lassen. Dazu wenden wir den SQL-Befehl \sqlil{SELECT ... FROM} auf die entsprechende Systemtabelle an.}%
\only<7>{\item<7-> Es gibt zwei Benutzerkonten: Das Systemadministratorkonto \textil{postgres} und den neuen Benutzer \textil{boss}.}%
\only<8>{\item<8-> Wir beenden die \psql-Session durch \textil{\\q} und \keys{\enter}.}%
\only<9>{\item<9-> Wir sind zurück im normalen Terminal.}%
\end{itemize}}%
...
\locateGraphic{6}{width=0.85\paperwidth}{graphics/user/psqlNewUser5getUsers}{0.075}{0.25}%
\locateGraphic{7}{width=0.85\paperwidth}{graphics/user/psqlNewUser6gotUsers}{0.075}{0.25}%
\locateGraphic{8}{width=0.85\paperwidth}{graphics/user/psqlNewUser7typeQuit}{0.075}{0.25}%
\locateGraphic{9}{width=0.85\paperwidth}{graphics/user/psqlNewUser8quit}{0.075}{0.25}%
\end{frame}%
%
\begin{frame}[t]%
...
\only<-4>{%
\begin{itemize}%
\item Alternativ können wir auch die SQL-Kommandos in eine Text-Datei schreiben.%
...
%
\item<13-> \bashil{-e}: Drucke alle erfolgreichen Befehle auf \pgls{stdout}%
%
...
%
\begin{frame}%
\frametitle{Erstellen einer Datenbank}%
\begin{itemize}%
...
\item<2-> Das machen wir genauso wie vorhin das Erstellen des Benutzers\uncover<3->{%
\begin{itemize}%
\item Wir können entweder im Terminal \psql\ öffnen und die entsprechenden SQL-Kommandos nacheinander eingeben\dots%
\item<4-> oder wir können die SQL-Kommandos in eine Datei schreiben und diese via \psql\ in einem Schritt ausführen.%
\end{itemize}%
}%
\item<5-> Zum letzen Mal werden wir beides anschauen.%
\item<6-> Danach werden wir nur noch mit Dateien arbeiten.%
\end{itemize}%
\end{frame}%
%
\begin{frame}[t]%
...
\uncover<-12>{%
\begin{itemize}%
\only<-1>{\item Wir loggen uns jetzt in unseren \postgresql-Server ein und erstellen die neue Datenbank Schritt-für-Schritt.}%
